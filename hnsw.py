...
def heuristic_improved_optimized(
    candidates, curr, k, distance_func, data, k_reduce=0.75, rand_edges=0.005
):
    candidates = sorted(candidates, key=lambda a: a[1])
    ...
    for c, curr_dist in candidates:
        if len(result) < k_reduce * k and (c not in result_indx_set):
            result.append((c, curr_dist))
            result_indx_set.add(c)

    num_shortcuts = int(k * (1 - k_reduce))
    neighbors = candidates[-num_shortcuts:]

    for c, curr_dist in neighbors:
        if c not in result_indx_set:
            result.append((c, curr_dist))
            result_indx_set.add(c)

    for c, curr_dist in random.choices(candidates, k=int(rand_edges * k)):
        if c not in result_indx_set:
            result.append((c, curr_dist))
            result_indx_set.add(c)
    ...
    def add(self, elem, ef=None):
        ...
        if point is not None:  # the HNSW is not empty, we have an entry point
            ...
            for layer in reversed(graphs[level:]):
                point, dist = self.beam_search(
                    graph=layer, q=elem, k=1, eps=[point], ef=1
                )[0]
            ...
            for layer in reversed(graphs[:level]):
                ...
                neighbors = self.neighborhood_construction(
                ...
                for j, dist in neighbors:
                    ...
                    neighbors_j = self.neighborhood_construction(
