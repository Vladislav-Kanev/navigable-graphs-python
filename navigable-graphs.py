...
class KmGraph(object):
    ...
    def beam_search(
        self, q, k, eps, ef, ax=None, marker_size=20, return_observed=False
    ):
        """
        q - query
    ...
    def brute_force_knn_search(self, k, x):
        ...
        return sorted(
            enumerate(self._vectorized_distance(x, self.data)), key=lambda a: a[1]
        )[:k]
    ...
    def plot_graph(self, ax, color, linewidth=0.5):
        ...
        for i in range(len(self.data)):
            for edge_end in self.edges[i]:
                ax.plot(
                    [self.data[i][0], self.data[edge_end][0]],
                    [self.data[i][1], self.data[edge_end][1]],
                    c=color,
                    linewidth=linewidth,
                )
...
def calculate_recall(kg, test, groundtruth, k, ef, m):
    if groundtruth is None:
        ...
        groundtruth = [
            [idx for idx, dist in kg.brute_force_knn_search(k, query)]
    ...
    for query, true_neighbors in tqdm(zip(test, groundtruth), total=len(test)):
        ...
        observed = [
            neighbor
            for neighbor, dist in kg.beam_search(
                query, k, entry_points, ef, return_observed=True
            )
        ]
...
def main():
    ...
    # Create KGraph
    kg = KmGraph(
        k=args.K, dim=args.dim, dist_func=KmGraph.l2_distance, data=train_data, M=args.M
    )

    # Calculate recall
    recall, avg_cal = calculate_recall(
        kg, test_data, groundtruth_data, k=args.k, ef=args.ef, m=args.m
    )
